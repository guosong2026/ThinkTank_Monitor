...
AI摘要支持在 Web 的“设置”页面直接填写火山方舟配置。需要提供：
...
部分来源的详情页对服务器请求返回 403（人机校验），如果只依赖抓取正文，就会出现"抓到了条目、拿不到摘要"的长期缺口。当前策略：

- **IISD**：出版物详情页受 Cloudflare 校验保护。解析器改为解析 `https://www.iisd.org/publications` 列表页的卡片，取回每条出版物的官方简介、类型和发布日期；AI 摘要先尝试详情页，读不到时使用列表页简介，并在摘要末尾标注"（依据官网列表页简介概括）"。
- **Land Use Policy**（ScienceDirect 期刊 RSS）：RSS 只提供题录，文章页返回 403。系统用链接中的 PII 在 Crossref 定位 DOI 和完整题录，再按 DOI 依次尝试 Crossref、OpenAlex、Semantic Scholar 的公开摘要：取到摘要时标注"（依据公开摘要数据库内容概括）"，只能拿到题录时标注"（依据题录信息概括，未获取原文摘要）"。缺少正文不再直接跳过摘要，也不会把题录概括当成原文结论。
- **其他来源保持原行为**：抓不到正文就不生成摘要，避免产生没有依据的摘要。只有已知会被拦截的域名（`iisd.org`、`sciencedirect.com`）或解析阶段确实带回简介/题录时才启用兜底。
- 兜底只发生在监测与补偿流程，不影响去重、推送状态和邮件内容；`summary_recovery.py` 对同一篇报告每 6 小时最多重试一次，因此上线后历史缺失摘要会逐步自动补齐。
- 导航链接过滤改为按 URL 路径片段匹配，`more`、`page` 等通用词不再从标题中间误判正常报告（例如 IISD 的《Making Electric Vehicles Work for More People》此前被丢弃）。
...
- **知识图谱**：分析近两个月已有AI摘要的报告，展示关键词、报告及关键词共现关系
...
| `/api/knowledge-graph` | GET | 获取知识图谱；`force=1`强制重建网络 |
| `/api/send_unsent` | POST | 发送未发送的报告邮件 |

### 7. 知识图谱更新策略

- 使用与AI摘要相同的火山方舟配置，从每篇AI摘要提取并保存10个中文关键词。
- 分析范围为最近两个日历月内录入数据库、且已有AI摘要的报告。
- 同义词、年份、月份、文献类型停用词、单字词和超长关键词会在本地统一处理。
- 单篇关键词按“标题 + AI摘要”的摘要指纹缓存到`report_kg_keywords`表；只有新增报告或摘要变化时才再次调用模型。
- 默认图谱在北京时间每周五03:00更新；服务错过调度时，会在周五后的首次访问补建。
- 设置`ENABLE_KG_SCHEDULER=0`可关闭后台周五任务，页面仍可按周缓存或手动刷新。
...
#### 8.3 数据库配置
- 配置文件：`reports.db`（默认）
- 主要包含`reports`、`settings`、`monitor_runs`和`report_kg_keywords`四张表
- 设置信息持久化存储，重启后保留
